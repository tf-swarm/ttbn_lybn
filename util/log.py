...
# _NOTSET = ''                  # white
# _DEBUG = '\033[1;35;40m'      # cyan
# _INFO = ''                    # white
# _WARN = '\033[1;36;40m'       # blue
# _ERROR = '\033[1;33;40m'      # yellow
# # FATAL = '\033[1;31;40m'     # red
# _FATAL = ''                   # white
# #_BI = ''                      # white
# _RECOVERY = '\033[0m'         # recovery
...
# logging.BI = logging.FATAL + 10
# logging.addLevelName(logging.BI, _' | B | ' + _BI)
...
class Logger(object):
    ...
    @classmethod
    def testlog(cls, *args):
        ct = time.time()
        fstr = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        fstr += str(ct).split('.')[1]
        fstr += '=====:'
        fstr += cls.__serialize_args(*args)
        fstr += "\n"
        file = os.path.dirname(Global.get_log_path()) + "/test.log"
    ...
    @classmethod
    def __to_string(cls, arg_object, string_within_quotos=False):
        """
        ...
        #     except:
        #         segment = repr(arg_object)
        #     return ('\'' + segment + '\'') if string_within_quotos else segment
        # elif isinstance(arg_object, (int, bool, long, float)):
        #     return str(arg_object)
        # elif isinstance(arg_object, (list, tuple)):
        #     segment = []
        #     for i in xrange(len(arg_object)):
        #         segment.append(cls.__to_string(arg_object[i], True))
        #     if isinstance(arg_object, list):
        #         return '[' + ', '.join(segment) + ']'
        #     else:
        #         return '(' + ', '.join(segment) + ')'
        # else:
        #     try:
        #         return repr(arg_object)
        #     except:
        #         traceback.print_exc()
        #         return "** serialize item failed **"
